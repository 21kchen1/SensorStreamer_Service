...
# CSV 文件路径
# csv_file = 'AUDIO3.csv'
csv_file = r'G:\Badminton\DataSetProcess\TrueData2\Man_Medium_BackhandTransition_LengYuanxing_2_1\AUDIO\Man_Medium_BackhandTransition_LengYuanxing_2_1_AUDIO.csv'
wav_file = 'output_audio1.wav'

# 设定参数
sampling_rate = 16000  # 采样率
channels = 1  # 单声道
sample_width = 2  # 16-bit 每个样本 4 字节
...
audio_samples = [value for index, value in enumerate(audio_samples) if index & 1]

maxValue, minValue = max(audio_samples), min(audio_samples)
print(f"maxValue: {maxValue} and minValue: {minValue}")
print(f"maxIndex: {audio_samples.index(maxValue)} and minIndex: {audio_samples.index(minValue)}")
# 将音频数据转换为 numpy 数组
